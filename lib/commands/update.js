...
export default async function update(args) {
  ...
  // Read package name from package.json (supports forks)
  const pkg = readJsonSafe(join(dirname(fileURLToPath(import.meta.url)), '..', '..', 'package.json'));
  const packageName = pkg?.name ?? 'reversa';
