...
import { runInstallPrompts, MIGRATION_AGENT_IDS, TRANSLATOR_AGENT_IDS, FORWARD_AGENT_IDS, PRICING_AGENT_IDS, PRODUCT_STRATEGY_AGENT_IDS, EVOLUTI_AGENT_IDS } from '../installer/prompts.js';
...
export default async function install(args) {
  ...
  const productStrategyInstalled = answers.agents.filter(a => PRODUCT_STRATEGY_AGENT_IDS.includes(a));
  const evolutiInstalled = answers.agents.filter(a => EVOLUTI_AGENT_IDS.includes(a));
  const discoveryInstalled = answers.agents.filter(a =>
    ...
    !PRODUCT_STRATEGY_AGENT_IDS.includes(a) &&
    !EVOLUTI_AGENT_IDS.includes(a)
  );
  ...
  if (productStrategyInstalled.length > 0) {
    console.log(`  ${chalk.cyan('Product Strategy:')}    ${productStrategyInstalled.length} agent(s)`);
  }
  if (evolutiInstalled.length > 0) {
    console.log(`  ${chalk.cyan('Evoluti Creation:')}    ${evolutiInstalled.length} agent(s)`);
  }
  console.log('');
  ...
  if (selectedEngines.length > 0) {
    ...
    if (productStrategyInstalled.length > 0) {
      const briefCommand = hasSlashEngine ? '/reversa-brief' : 'reversa-brief';
      const evolveCommand = hasSlashEngine ? '/reversa-evolve' : 'reversa-evolve';
      console.log(chalk.cyan(`  -> To create an LLM-ready repo brief, run ${briefCommand}; to design an expanded product, run ${evolveCommand}`));
    }
    if (evolutiInstalled.length > 0) {
      const revolutiCommand = hasSlashEngine ? '/revoluti' : 'revoluti';
      console.log(chalk.cyan(`  -> To create a new product from analysis, run ${revolutiCommand} after ${evolveCommand}`));
    }
  }
  console.log('');
}
